...
public class OrderManagementController {
    ...
    @FXML private TableColumn<Order, BigDecimal> colOrderTotalAmount;
    ...
    private void loadOrderRatingChartData() {
        ...
        try {
            LanguageManager lm = LanguageManager.getInstance(); // Lấy LanguageManager để dịch text
            ...
            if (orderRatingPieChart != null) { // Đặt tiêu đề lỗi nếu có
                orderRatingPieChart.setTitle(LanguageManager.getInstance().getString("error.load.ratingChart"));
            }
        }
    }
    ...
    private void setupOrderTableColumns() {
        ...
        colOrderTotalAmount.setCellFactory(tc -> new TableCell<Order, BigDecimal>() {
            @Override
            protected void updateItem(BigDecimal item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(currencyFormatter.format(item.doubleValue())); // Sử dụng currencyFormatter đã khai báo
                }
                setAlignment(Pos.CENTER_RIGHT); // Căn phải cho dễ đọc
            }
        });
    ...
    private void displayOrderDetail(Order order) {
        ...
        lblDetailTotalAmount.setText(order.getTotalAmount() != null ? currencyFormatter.format(order.getTotalAmount()) : "0 đ");
